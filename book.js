document.addEventListener("DOMContentLoaded", () => {
    ...
    const displayFlights = (from, to) => {
        ...
        if (results.length > 0) {
            results.forEach((flight) => {
             ...
             // Add event listeners for the "Generate Boarding Pass" buttons
             document.querySelectorAll(".generate-pass-btn").forEach((btn) => {
                btn.addEventListener("click", (e) => {
                    const flightIndex = e.target.dataset.index;
                    generateBoardingPass(results[flightIndex]);
                });
            }); 
        } else {
            const li = document.createElement("li");
    ...
    const generateBoardingPass = (flight) => {
        boardingPassDiv.innerHTML = `
            <div class="boarding-pass">
                ...
                <p><strong>Distance:</strong> ${flight.distance}</p>
